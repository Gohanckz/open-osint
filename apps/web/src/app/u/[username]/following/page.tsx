...
export default async function FollowingPage({
  ...
  const following = user.privacyMode
    ? []
  ...
  return (
    <FollowList
      title={fill(t.profile.followingListTitle, { name: user.displayName })}
      backHref={`/u/${user.username}`}
      backLabel={`@${user.username}`}
      users={following.map((f) => f.following)}
      privacy={user.privacyMode}
      emptyTitle={t.profile.notFollowingTitle}
      emptyDescription={fill(t.profile.notFollowingDesc, { name: user.displayName })}
